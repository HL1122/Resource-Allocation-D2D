...
class Agent(BaseModel):
    def __init__(self, config, environment, sess):
        ...
        self.action_all_with_power_training = np.zeros([20, 3, 2],dtype = 'int32')   # this is actions that taken by D2D links with power
        self.reward = []
    ...
    # ===============
        ...
        load_remaining = np.asarray([self.env.individual_time_limit[idx[0],idx[1]] / self.env.D2D_limit])
        #print('shapes', time_remaining.shape,load_remaining.shape)
        return np.concatenate((cellular_channel, D2D_interference, D2D_channel, NeiSelection, time_remaining, load_remaining))#,time_remaining))
        ...
        for self.step in (range(0, 40000)): # need more configuration
            ...
            if (self.step % 2000 == 0) and (self.step > 0):
                ...
                for game_idx in range(number_of_game):
                    self.env.new_random_game(self.num_devices)
                    test_sample = 200
                    Rate_list = []
    ...
    def play(self, n_step = 100, n_episode = 100, test_ep = None, render = False):
        ...
        for game_idx in range(number_of_game):
            ...
            print('Mean of the cellular rate is ', np.mean(cellular_Rate_list[0:game_idx] ))
            print('Mean of Fail percent is ',percent, np.mean(Fail_percent_list[0:game_idx]))
            # print('action is that', action_temp[0,:])
