...
class D2Dchannels:              
    ...
    def __init__(self, n_Dev, n_RB):
        self.t = 0
        self.h_bs = 1.5     # height of base station
        self.h_ms = 1.5  
        self.fc = 2     # carrier frequency
        self.decorrelation_distance = 10    # decorrelation distance (for shadowing)
        self.shadow_std = 3     # shadow standard
    ...
    def update_shadow(self, delta_distance_list):
        ...
        if len(delta_distance_list) == 0: 
            self.Shadow = np.random.normal(0,self.shadow_std, size=(self.n_Dev, self.n_Dev))
        else:
            self.Shadow = np.exp(-1*(delta_distance/self.decorrelation_distance)) * self.Shadow +\
                         np.sqrt(1 - np.exp(-2*(delta_distance/self.decorrelation_distance))) * np.random.normal(0, self.shadow_std, size = (self.n_Dev, self.n_Dev))
...
class Environ:
    ...
    def renew_channels_fastfading(self):   
        ...
        cellular_channels_with_fastfading = np.repeat(self.V2I_channels_abs[:, np.newaxis], self.n_RB, axis=1)
