...
def main():
    ...
    number_of_game = 500
    n_step = 1000
    cellular_Rate_List = np.zeros([number_of_game, n_step])
    ...
    for game_idx in range(number_of_game):
        ...
        for i in range(n_step):
            ...
            cellular_Rate_List[game_idx, i] = np.sum(reward)
            Fail_Percent[game_idx, i] = percent
        print(np.sum(reward))
        print ('percentage here is ', percent)
    print ('The number of devices are ', n)
    print ('mean of V2I rate is that ', np.mean(cellular_Rate_List))
